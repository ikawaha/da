...
func TestDaBuildAndSearch01(t *testing.T) {
	...
	if _, ok := d.Find(strings.NewReader("")); ok {
...
func TestDaBuildAndSearch03(t *testing.T) {
	...
	for _, k := range keywords {
		if _, ok := d.Find(strings.NewReader(k)); !ok {
...
func TestDaBuildAndCommonPrefixSearch01(t *testing.T) {
	...
	if ids, lens := d.CommonPrefixSearch(strings.NewReader("")); len(ids) != 0 || len(lens) != 0 {
...
func TestDaBuildWithIdsAndPrefixSearch03(t *testing.T) {
	...
	for key, expectedId := range h {
		if id, ok := d.Find(strings.NewReader(key)); !ok || id != expectedId {
...
func TestDaBuildAndPrefixSearch01(t *testing.T) {
	...
	if id, ok := d.PrefixSearch(strings.NewReader("")); ok {
...
func TestDaBuildAndPrefixSearch03(t *testing.T) {
	...
	id, ok := d.PrefixSearch(strings.NewReader("電気通信大学大学院電気通信学研究科"))
	if !ok {
...
func BenchmarkCommonPrefixSearch01(b *testing.B) {
	keywords := []string{
		"電気",                //1
		"電気通信",              //2
		"電気通信大学",            //3
