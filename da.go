...
func (d *DoubleArray) search(r Reader) (p, q, i int, ok bool) {
	var size int
	if size = r.Len(); size == 0 {
		return
	}
	bufLen := len(*d)
	for i = 0; i < size; i++ {
...
func (d *DoubleArray) Find(r Reader) (id int, ok bool) {
	p, q, _, ok := d.search(r)
	if !ok {
		return
	}
...
func (d *DoubleArray) CommonPrefixSearch(r Reader) (ids, lens []int) {
	var p, q int
	bufLen := len(*d)
	for i, end := 0, r.Len(); i < end; i++ {
		p = q
		b, _ := r.ReadByte()
		q = (*d)[p].Base + int(b)
		if q >= bufLen || (*d)[q].Check != p {
